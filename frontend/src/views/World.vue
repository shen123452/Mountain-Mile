...
<template>
  ...
  <section v-else class="world-page">
    ...
    <div class="world-main">
      ...
      <div v-if="selected" class="focus-bar"><strong>为这座山投入时间</strong><label><input v-model.number="focusMinutes" type="number" min="1" max="240" :disabled="!!focusId" /> 分钟</label><button type="button" :disabled="focusBusy" @click="startFocus">{{ focusBusy ? '准备中…' : '开始专注' }}</button></div>
      <div class="island-stage" :style="{ backgroundColor: selected ? palettes[selected.palette_variant].sky : '#e8eee4' }"><VoxelIsland v-if="selected" :key="selected.id" :tiles="tiles" :unlocked-count="selected.unlocked_count" :palette="palettes[selected.palette_variant]" :selected-order="selectedTile?.unlockOrder ?? null" @select="selectedTile = $event" /><p v-else class="empty-stage">你的第一座山屿即将出现在这里。</p><p v-if="selectedTile" class="tile-detail">{{ tileLabels[selectedTile.kind] }} · 第 {{ selectedTile.unlockOrder }} 块地块 · 海拔 {{ selectedTile.elevation.toFixed(1) }}</p></div>
    </div>
...
.tile-detail{position:absolute;right:1rem;bottom:1rem;margin:0;padding:.55rem .75rem;border:1px solid #c6d9c7;border-radius:8px;color:#315d4e;background:#f7faf0e8;font-size:.78rem;box-shadow:0 8px 18px #315d4e18}.focus-bar{display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding:0 3vw 18px;color:#315d4e}.focus-bar label{margin:0;display:flex;align-items:center;gap:6px}.focus-bar input{width:70px;padding:6px;border:1px solid #9bb8a8;border-radius:6px}.focus-bar button{padding:7px 12px;border:0;border-radius:7px;background:#285d4e;color:#fff;cursor:pointer}.focus-bar button:disabled{opacity:.55}
</style>
