    ...
    <aside class="run-rail" aria-label="运行记录">
      ...
      <div class="quick-prompts"><button v-for="prompt in quickPrompts" :key="prompt" type="button" @click="applyPrompt(prompt)">{{ prompt }}</button></div>
        ...
        <div v-for="run in runs" :key="run.id" class="run-item" :class="{ active: selected?.id === run.id }" :data-status="run.status">
          <span class="run-goal-text" role="button" @click="open(run.id)">{{ run.goal }}</span><small>{{ labels[run.status] }}</small><span class="run-ops"><button type="button" class="task-op" title="归档:从列表隐藏但保留记录" @click="archiveRun(run)">归档</button><button type="button" class="task-op danger" title="删除:含全部轨迹,不可恢复" @click="deleteRun(run)">删除</button></span>
        </div>
      </div>
    </aside>
