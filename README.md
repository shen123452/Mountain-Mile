...
M7 提供 `/agent/runs/:id/stream` SSE 实时时间线：思考、工具调用与结果全程流式可见并持久化为可回放事件，断线后按事件 ID 补齐；支持中断（状态落库为 `cancelled`）与审批后断点续跑。

M8 落地五角色协作（规划者 / 执行者 / 复盘者 / 记忆官 / 向导）与 `delegateToRole` 委派，三档自主程度（L0–L2）+ 工具风险分级生效；`/auth/autonomy` 可切换档位，工作台展示当前角色与审批策略。

M9 上线 `/knowledge`：上传 md/pdf/docx/txt 建立个人知识库（分块 + 百炼 embedding + pgvector 检索），长期记忆自动去重合并（归一化 + 余弦 ≥0.92），支持查看 / 新增 / 编辑 / 删除；每日凌晨自动清理超过 90 天未使用的低重要记忆并留存审计通知。

M10 上线 `/observatory` 观测台：token 成本趋势（估算）、工具成功率 / 耗时、运行轨迹回放；SM-2 间隔复习进入今日任务，`/knowledge` 页可直接评分（陌生 / 模糊 / 掌握）；六个分析工具（打卡查询、学习统计、安排复习、专注节律、达成预测、过载检测）全部接通。
