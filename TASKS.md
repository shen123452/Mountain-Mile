...
## 已完成：M6 对话 + 报告 + APScheduler 调度

- [x] 会话和消息持久化；Agent 工作台提供本地可用的对话入口，配置 DashScope 后自动请求模型。
- [x] 日报/周报统计接口，汇总专注分钟、专注次数、打卡天数和活跃目标。
- [x] Agent cron 调度配置和 APScheduler 生命周期；定时任务会创建运行记录，未配置模型时记录失败通知而不阻塞服务。
- [x] PostgreSQL 迁移、会话/报告/调度 API 测试及前端构建通过。

下一任务为 M7；目标与验收门见 `docs/开发文档.md` 第 16 节。

## 已完成：M7 SSE 流式时间线 + 中断 / 断点续跑

- [x] Agent 步骤持久化为可回放事件，`/agent/runs/:id/stream` 支持 SSE 订阅和断线后按事件 ID 补齐。
- [x] 运行后台执行，创建接口立即返回；工作台显示实时事件并支持中断。
- [x] 中断状态持久化为 `cancelled`，审批恢复继续沿用持久化消息历史。

下一任务为 M8；目标与验收门见 `docs/开发文档.md` 第 16 节。

## 已完成：M8 多角色 + 自主档位 / 权限

- [x] Planner、Executor、Reflector、Curator、Scout 五角色按请求路由；每个角色只看到自己的工具白名单。
- [x] 支持 `delegateToRole` 角色委派，委派过程写入 AgentStep/SSE 时间线并切换后续角色上下文。
- [x] L0/L1/L2 自主档位生效：只读自动执行，低风险写入在 L1+ 自动执行，高风险写入始终审批；新增 `/auth/autonomy` 设置接口。
